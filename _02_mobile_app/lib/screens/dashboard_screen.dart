  ...
  void initState() {
    ...
    _sub = _mqtt.telemetryStream.listen((payload) {
      ...
      setState(() {
        ...
        _lastSeenByEngine[payload.engineId] = DateTime.now();
        _connectionState = 'Live';
        final h = _historyByEngine.putIfAbsent(payload.engineId, () => {});
        payload.currentSensorReadings.forEach((k, v) {
          final list = h.putIfAbsent(k, () => []);
          list.add(v);
